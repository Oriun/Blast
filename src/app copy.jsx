...
const mainContext = createTunnel();
  ...
  return (
    ...
    >
      ...
      {n !== 1 ? (
        ...
        <Box
          className={n.toString() + $.toString()}
          id={"alabama" + n.toString()}
        />
      )}
    </span>
  );
...
const App = ({ n = 0 }, $) => {
  const ctx = useState(
    $,
    "Current Task : vDiff function => children diff + attributes separate diff"
  );
  console.log("ctx", ctx);
  ...
  return (
    <div intl={[NoOut]}>
      ...
      <br />
      ToDo : update from specific node and not the root and propagate to
      children
      {context}
      <br />
      <br />
